...
- [x] **Zensus 2022 in der Pipeline** (2026-09-25, `unfallkarte census check|build`, census.py)
      — die Kacheln waren eine Kopie aus routing_bulk ohne Rezept. Herkunft rekonstruiert:
      npgeo-Hub der vfdb (Gitter 100 m + 1 km mit ~45 Merkmalen) -> erreichbarad
      `1aa_add_plz_to_pop_ger_v03_regiosta.ipynb` (PLZ `within`, RegioStaR 2023 per `ags`,
      INNER) -> routing_bulk `prepare_population.ipynb` (tippecanoe). Rohdaten als Archivkopien in
      `pipeline/data/raw/census/` (Quellen in sources.yaml). **Reproduktion bewiesen:** Parquet
      in allen Attributen identisch (Geometrie bis auf die letzte Binärstelle), alle 881
      Kacheln byte-gleich. Danach verbessert: 100 m nur ab z11 (z11–12, full-detail 13 ->
      Zellen bis z18 rechteckig statt auf ein 6-m-Raster gesprungen), 9 statt ~50 Felder als
      echte Zahlen (PLZ + RegioStaR bewusst drin), dazu 1 km für die Übersicht z8–10.
      393 MB -> 149 + 19 MB; größte Kachel 1.959 -> 205 KB (z11), Median 234 -> 21 KB.
